/*!
Используется форк jquery плагина с незначительными изменениями
для отслеживания dirty состояния отдельных полей в дополнение
к отслеживанию состояния всей формы целиком
@link https://github.com/dshmanatov/jquery.dirtyforms/pull/1/files
...
(function (factory) {
    ...
    var setFieldStatus = function ($field, ignoreSelector) {
        ...
        if ($field.is(':radio[name]')) {
            ...
            dirty = isFieldDirty($field, ignoreSelector);
            setDirtyStatus($field, dirty);
        }
